...
def create_MAC_tag(mac_key: bytes, message: bytes, mask: bytes) -> bytes:
    ...
    print("server h: ", h)
    print("server c: ", c)
    print("server K^2: ", (K**2) % p)
    print("server K^3: ", (K**3) % p)
    print("server K: ", K)
    print("-------------------------------------------------------------------")
...
def derive_keys(key: bytes):
    cipher_key: bytes = HKDF(key, 16, salt=b"", hashmod=SHA256,
                             context=b"exodia-enc", num_keys=1)  # type: ignore
    mac_key: bytes = HKDF(key, 32, salt=b"", hashmod=SHA256,
                          context=b"exodia-mac", num_keys=1)  # type: ignore
...
def encrypt(key: bytes, message: bytes, nonce: bytes) -> Tuple[bytes, bytes]:
    ...
    # print("server mask: ", mask, "integer value: ",
    #       int.from_bytes(mask, "big") % p)
    ...
    def handle_decrypt(self, msg):
        ...
        try:
            ...
            if plaintext == "Give me a flag!":
                self.send_message(
                    {"res": f"Wait. That's illegal. {self.flag}"})
            else:
                self.send_message(
                    {"res": f"We'll take that into consideration. *throws plaintext into the bin*"})
        except (KeyError, ValueError, TypeError) as e:
            self.send_message(
                {"error": f"Invalid parameters: {type(e).__name__} {e}"})
